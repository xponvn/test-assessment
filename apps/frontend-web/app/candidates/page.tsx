...
import { CandidateRowData, transformListCandidates } from './utils/helps';
  ...
  const [filterVariants, setFilterVariants] =
    useState<GetCandidatesQueryVariables>({
      ...
      filters: {
        ...
        position: { eq: undefined },
      },
    });
  useEffect(() => {
    setFilterVariants({
      ...filterVariants,
      filters: { ...filterVariants.filters, email: { containsi: searchKey } },
    });
    ...
    <div className="bg-neutral-bg h-full">
      <div className="container mx-auto flex items-center justify-between pt-[45px] px-6">
        ...
        <Button
          ...
          type="button"
          children={<span className='text-neutral-text-primary text-13'>Add Candidate</span>}
        />
      </div>
              ...
              render: (row) => (
                <div className="flex items-center gap-3">
                  <span
                    ...
                    onClick={() => alert('Edit')}
                  >
                    <Icon name="edit" className="text-neutral-placeholder gap-3" />
                  </span>
                  <span
                    className="cursor-pointer"
                    onClick={() => onRemoveItem(row.id)}
                  >
                    <Icon name="remove" className="text-neutral-placeholder gap-3" />
                  </span>
                </div>
              ),
            },
