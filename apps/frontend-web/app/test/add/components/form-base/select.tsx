  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <select
        ref={ref}
        {...reset}
        className={clsx("border border-solid py-[5px] px-3 outline-none text-15 leading-24 text-neutral-text-primary min-h-[36px] capitalize", {
      ...
      >
        ...
        {options.map((item, index) => {
          return <option value={item.value} key={index} className="capitalize">{item.label}</option>
        })}
      </select>
