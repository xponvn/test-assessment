      ...
      <div className="px-6 py-4 bg-neutral-table-header">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <p className="font-medium text-neutral-text-primary text-13 leading-24">Question {index + 1}</p>
            <span className="mx-1 text-neutral-placeholder text-13 leading-20">({getPoint(level)} point)</span>
        ...
        <p className="mt-2 font-medium leading-6 text-15 text-neutral-text-primary">{content}</p>
      </div>

      {/** Answers */}
      <div className="px-6 py-4 border-t border-solid border-neutral-divider bg-neutral-white">
        <div className="flex leading-5 text-neutral-text-secondary text-13">Type of answer: <span className="ml-1 font-medium capitalize text-neutral-text-primary">{getQuestionType(type)}</span></div>
        {type !== QuestionType.FreeText && <div className="flex flex-col gap-2 mt-4">
          {answers.map((aItem, index) => {
            return <div key={index} className={clsx("flex items-center", {
              ...
              <p className="ml-2 font-medium leading-5 text-13">{aItem.content}</p>
            </div>
          })}
        </div>}
      </div>
